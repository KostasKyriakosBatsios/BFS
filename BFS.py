...
def bfs(graph, beggining, goal):
    visited = list() # Keeping track of visited nodes
    queue = [(beggining, [beggining])] # Storing nodes and it's path (Queue is FIFO = First In First Out)

    while queue:
        current_node, path = queue.pop(0) # Getting the current node and it's path
...
result, path, visited = bfs(gadj, root, target) # Executing the Breadth First Search (BFS)
